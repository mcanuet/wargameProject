...
def tournois(population,IA=bebeBeel,jeu=Jeu,plateaux=Plateau(20,20),nbVersions=5,save=None,backData=False,joinScore=False,progressbar=False,*args,**kw):  # *args et **kw pour palier aux potentiels problèmes via l'Algo G (Gestion de l'ensemble des arguments à repartager entre les fonctions de sélection, mutation et croisement)
    """
    ...
    :param progressbar: Affiche, dans la console, une barre de progression sur l'avancée du tournois.
    :type progressbar: boolean
    """

    data = {}       #Plus pratique pour passer à du JSON via le module json
    graine=0
    var_progressbar = [0,int(((len(population)**2)-len(population))/2)*nbVersions]
...
##                        victoiresi+=1
                    ...
                    #Progressbar dans la console
                    if progressbar:
                        progression_progressbar = round(var_progressbar[0]/var_progressbar[1],2)
                        #print("PP:",progression_progressbar)
                        #Cas particulier du début
                        if var_progressbar[0]==0:print("Progression du tournois : 0%",end="")
                        #Sinon, si dans la proportionnalité il y a une modification (ex: passage du 1% à 2%) alors...
                        elif progression_progressbar != round((var_progressbar[0]-1)/var_progressbar[1]):
                            if round((var_progressbar[0])/var_progressbar[1],2) < 0.75 and round((var_progressbar[0]+1)/var_progressbar[1],2) >= 0.75:print("75%",end="")
                            elif round((var_progressbar[0])/var_progressbar[1],2) < 0.5 and round((var_progressbar[0]+1)/var_progressbar[1],2) >= 0.5:print("50%",end="")
                            elif round((var_progressbar[0])/var_progressbar[1],2) < 0.25 and round((var_progressbar[0]+1)/var_progressbar[1],2) >= 0.25:print("25%",end="")
                            else:print("•",end="")
                        var_progressbar[0]+=1
                        if var_progressbar[0] == var_progressbar[1]:print("100%")
    ...
    triScore.reverse()
    if joinScore:
        population = [{"score":i,"element":population[dicoTri[i]]} for i in triScore]
    else:
        population = [population[dicoTri[i]] for i in triScore]
